# modified from https://github.com/ClementPinard/SfmLearner-Pytorch
...
def predict_disp(in_planes, n_classes=19):
    return nn.Sequential(
        nn.Conv2d(in_planes, n_classes, kernel_size=1, padding=0),
        nn.ReLU(inplace=True)  # different from dispnet for depth
    )
...
class dispnet_seg(nn.Module):

    def __init__(self, alpha=10.0, beta=0, n_classes=19):
        super(dispnet_seg, self).__init__()
        ...
        self.iconv3 = conv(self.n_classes + upconv_planes[4] + conv_planes[1], upconv_planes[4])
        self.iconv2 = conv(self.n_classes + upconv_planes[5] + conv_planes[0], upconv_planes[5])
        self.iconv1 = conv(self.n_classes + upconv_planes[6], upconv_planes[6])

        self.predict_disp4 = predict_disp(upconv_planes[3], n_classes=self.n_classes)
        self.predict_disp3 = predict_disp(upconv_planes[4], n_classes=self.n_classes)
        self.predict_disp2 = predict_disp(upconv_planes[5], n_classes=self.n_classes)
        self.predict_disp1 = predict_disp(upconv_planes[6], n_classes=self.n_classes)
    ...
    def forward(self, x):
        ...
        disp4_up = crop_like(F.interpolate(disp4, scale_factor=2, mode='bilinear', align_corners=True), out_conv2)
        ...
        disp3_up = crop_like(F.interpolate(disp3, scale_factor=2, mode='bilinear', align_corners=True), out_conv1)
        concat2 = torch.cat((out_upconv2, out_conv1, disp3_up), 1)
        out_iconv2 = self.iconv2(concat2)
        disp2 = self.alpha * self.predict_disp2(out_iconv2) + self.beta

        out_upconv1 = crop_like(self.upconv1(out_iconv2), x)
        disp2_up = crop_like(F.interpolate(disp2, scale_factor=2, mode='bilinear', align_corners=True), x)
